...
func TestSendTelemetryPing(t *testing.T) {
	...
	if receivedUserAgent != "AgilePanel-CLI/1.0.0" {
		t.Errorf("expected User-Agent AgilePanel-CLI/1.0.0, got %s", receivedUserAgent)
	}
	...
	if receivedPayload.Version != "1.0.0" {
		t.Errorf("expected Version 1.0.0, got %s", receivedPayload.Version)
	}
}
